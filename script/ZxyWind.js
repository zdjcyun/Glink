//深圳智翔宇二维风速风向仪解析
//生成采集指令
function ZxyWind_generateInstruct(sensorAddr, sensorType) {
    var partInstruct = sensorAddr + '0300000004';
    var crc16 = toModbusCRC16(partInstruct, true); //true代表低位在前，false代表高位在前。
    return partInstruct + crc16;
}

// 解析回复指令(默认已去除所有空格)
function ZxyWind_parserInstruct(msg) {
    // 判断CRC16是否一致
    if (!ZxyWind_vaildMsgLength(msg)){
        return "ERROR01" //ERROR01 代表长度不够 异常
    }
    if(!ZxyWind_vaildMsgCrc16(msg)){
    ...
    var dataX = ZxyWind_reverse(dataBody.substring(0, 8));
    var dataY = ZxyWind_reverse(dataBody.substring(8));
...
function ZxyWind_vaildMsgLength(msg) {
...
function ZxyWind_vaildMsgCrc16(msg) {
