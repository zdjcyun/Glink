//长沙鲲鹏激光测距解析
//生成采集指令
function KpLaser_generateInstruct(sensorAddr, sensorType) {
    var partInstruct = sensorAddr + '03000F0002';
    var crc16 = toModbusCRC16(partInstruct, true); //true代表低位在前，false代表高位在前 。
    return partInstruct + crc16;
}

// 解析回复指令(默认已去除所有空格)
function KpLaser_parserInstruct(msg) {
    // 判断CRC16是否一致
    if (!KpLaser_vaildMsgLength(msg)) {
        return "ERROR01" //ERROR01 代表长度不够 异常
    }
    if (!KpLaser_vaildMsgCrc16(msg)) {
...
function KpLaser_vaildMsgLength(msg) {
...
function KpLaser_vaildMsgCrc16(msg) {
