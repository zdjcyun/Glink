//长沙鲲鹏温湿度计解析
//生成采集指令
function KpTemHum_generateInstruct(sensorAddr, sensorType) {
    var partInstruct = sensorAddr + '0300000002';
    var crc16 = toModbusCRC16(partInstruct, true); //true代表低位在前，false代表高位在前。
    return partInstruct + crc16;
}

// 解析回复指令(默认已去除所有空格)
function KpTemHum_parserInstruct(msg) {
    // 判断CRC16是否一致
    if (!KpTemHum_vaildMsgLength(msg)){
        return "ERROR01" //ERROR01 代表长度不够 异常
    }
    if (!KpTemHum_vaildMsgCrc16(msg)){
    ...
    var hum = parseInt(dataBody.substring(0, 4), 16) / 10;
    // 将长整数转换成16位有符号整数
	var arrTem = new Int16Array([parseInt(dataBody.substring(4), 16)])
    var tem = arrTem[0] / 10;
    var result = {
        x : tem,
		y : hum
...
function KpTemHum_vaildMsgLength(msg) {
...
function KpTemHum_vaildMsgCrc16(msg) {
