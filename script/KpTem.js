//长沙鲲鹏温度传感器解析
//生成采集指令
function KpTem_generateInstruct(sensorAddr, sensorType) {
...
// 解析回复指令(默认已去除所有空格)
function KpTem_parserInstruct(msg) {
    // 判断CRC16是否一致
    if (!KpTem_vaildMsgLength(msg)){
        return "ERROR01" //ERROR01 代表长度不够 异常
    }
    if(!KpTem_vaildMsgCrc16(msg)){
        return "ERROR02" //ERROR02 代表CRC16校验不一致 异常
    }
    var sensorAddr = msg.substring(0, 2);
    // 通用数据格式需除去地址位，功能码，长度位一起3个字节
    var dataTemp = msg.substring(6, msg.length - 4);
	// 将长整数转换成16位有符号整数
	var resultTemp = new Int16Array([parseInt(dataTemp, 16)])[0] / 10;
    var result = {
		x : resultTemp
    }
...
function KpTem_vaildMsgLength(msg) {
...
function KpTem_vaildMsgCrc16(msg) {
