//长沙位移类传感器解析
//生成采集指令
function KpCommon_generateInstruct(sensorAddr, sensorType) {
...
// 解析回复指令(默认已去除所有空格)
function KpCommon_parserInstruct(msg) {
    // 判断CRC16是否一致
    if (!KpCommon_vaildMsgLength(msg)){
        return "ERROR01" //ERROR01 代表长度不够 异常
    }
    if(!KpCommon_vaildMsgCrc16(msg)){
...
function KpCommon_vaildMsgLength(msg) {
...
function KpCommon_vaildMsgCrc16(msg) {
