//长沙弦式水位计（渗压计）转Modbus协议解析
//生成采集指令
function KpWireWaterLevel_generateInstruct(sensorAddr, sensorType) {
...
// 解析回复指令(默认已去除所有空格)
function KpWireWaterLevel_parserInstruct(msg) {
    // 判断CRC16是否一致
    if (!KpWireWaterLevel_vaildMsgLength(msg)){
        return "ERROR01" //ERROR01 代表长度不够 异常
    }
    if(!KpWireWaterLevel_vaildMsgCrc16(msg)){
	...
	// 水位计（渗压计）K1值外输，K2值固定为0.0005383；
	var resultX = ((0.0005383 * Math.pow(rateX, 2) * 1000) / 9.8).toFixed(10);
...
function KpWireWaterLevel_vaildMsgLength(msg) {
...
function KpWireWaterLevel_vaildMsgCrc16(msg) {
