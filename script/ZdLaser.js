//中大激光测距仪解析
//生成采集指令
function ZdLaser_generateInstruct(sensorAddr, sensorType) {
    var partInstruct = sensorAddr + '11095A444A43' + sensorType +'20190809';
...
// 解析回复指令(默认已去除所有空格)
function ZdLaser_parserInstruct(msg) {
    // 判断CRC16是否一致
    if (!ZdLaser_vaildMsgLength(msg)){
        return "ERROR01"; //ERROR01 代表长度不够 异常
    }
    if(!ZdLaser_vaildMsgCrc16(msg)){
	...
	// 单个数据需要处理时，该值默认为 0
	var resultY = 0;
	// 如果消息体的长度等于8个字节，即有两个数据需要处理，后一个4字节数据为异常值
	if(dataBody.length === 16){
		var dataY = dataBody.substring(8);
		resultY = hexToSingle(dataY);
	}
...
function ZdLaser_vaildMsgLength(msg) {
...
function ZdLaser_vaildMsgCrc16(msg) {
