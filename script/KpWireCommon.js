//长沙振弦式转Modbus协议通用解析（K值外输）
//生成采集指令
function KpWireCommon_generateInstruct(sensorAddr, sensorType) {
...
// 解析回复指令(默认已去除所有空格)
function KpWireCommon_parserInstruct(msg) {
    // 判断CRC16是否一致
    if (!KpWireCommon_vaildMsgLength(msg)){
        return "ERROR01" //ERROR01 代表长度不够 异常
    }
    if(!KpWireCommon_vaildMsgCrc16(msg)){
	...
	var resultX = Math.pow(rateX, 2).toFixed(10);
...
function KpWireCommon_vaildMsgLength(msg) {
...
function KpWireCommon_vaildMsgCrc16(msg) {
