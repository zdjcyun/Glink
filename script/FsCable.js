//江西飞尚拉线位移计解析
//生成采集指令
function FsCable_generateInstruct(sensorAddr, sensorType) {
...
// 解析回复指令(默认已去除所有空格)
function FsCable_parserInstruct(msg) {
    // 判断CRC16是否一致
    if (!FsCable_vaildMsgLength(msg)) {
        return "ERROR01" //ERROR01 代表长度不够 异常
    }
    if (!FsCable_vaildMsgCrc16(msg)) {
...
function FsCable_vaildMsgLength(msg) {
...
function FsCable_vaildMsgCrc16(msg) {
