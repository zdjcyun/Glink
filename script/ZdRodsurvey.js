//中大导轮测斜仪解析
//生成采集指令
function ZdRodsurvey_generateInstruct(sensorAddr, sensorType) {
...
// 解析回复指令(默认已去除所有空格)
function ZdRodsurvey_parserInstruct(msg) {
    // 判断CRC16是否一致
    if (!ZdRodsurvey_vaildMsgLength(msg)){
        return "ERROR01" //ERROR01 代表长度不够 异常
    }
    if(!ZdRodsurvey_vaildMsgCrc16(msg)){
    ...
    // 通用数据格式需除去ZDJC+类型共5个字节，再加上地址位，功能码，长度位一起3个字节，以及温度位4个字节
...
function ZdRodsurvey_vaildMsgLength(msg) {
...
function ZdRodsurvey_vaildMsgCrc16(msg) {
