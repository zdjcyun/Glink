//上海盛迪静力水准仪解析
//生成采集指令
function SdStaticLevel_generateInstruct(sensorAddr, sensorType) {
    var partInstruct = sensorAddr + '0400000002';
...
// 解析回复指令(默认已去除所有空格)
function SdStaticLevel_parserInstruct(msg) {
    // 判断CRC16是否一致
    if (!SdStaticLevel_vaildMsgLength(msg)){
        return "ERROR01" //ERROR01 代表长度不够 异常
    }
    if(!SdStaticLevel_vaildMsgCrc16(msg)){
...
function SdStaticLevel_vaildMsgLength(msg) {
...
function SdStaticLevel_vaildMsgCrc16(msg) {
