//武汉承拓激光测距解析
//生成采集指令
function CtLaser_generateInstruct(sensorAddr, sensorType) {
    var partInstruct = sensorAddr + '0320010002';
    var crc16 = toModbusCRC16(partInstruct, true); //true代表低位在前，false代表高位在前 。
    return partInstruct + crc16;
}

// 解析回复指令(默认已去除所有空格)
function CtLaser_parserInstruct(msg) {
    // 判断CRC16是否一致
    if (!CtLaser_vaildMsgLength(msg)) {
        return "ERROR01" //ERROR01 代表长度不够 异常
    }
    if (!CtLaser_vaildMsgCrc16(msg)) {
    ...
    var resultX = parseInt(dataBody, 16) / 10;
...
function CtLaser_vaildMsgLength(msg) {
...
function CtLaser_vaildMsgCrc16(msg) {
