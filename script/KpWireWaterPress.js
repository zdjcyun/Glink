//长沙弦式渗压计转Modbus协议解析
//生成采集指令
function KpWireWaterPress_generateInstruct(sensorAddr, sensorType) {
...
// 解析回复指令(默认已去除所有空格)
function KpWireWaterPress_parserInstruct(msg) {
    // 判断CRC16是否一致
    if (!KpWireWaterPress_vaildMsgLength(msg)){
        return "ERROR01" //ERROR01 代表长度不够 异常
    }
    if(!KpWireWaterPress_vaildMsgCrc16(msg)){
	...
	// 渗压计K1值外输，K2值固定为0.0005383 此处做特殊处理；
	var resultX = (0.0005383 * Math.pow(rateX, 2)).toFixed(10);
...
function KpWireWaterPress_vaildMsgLength(msg) {
...
function KpWireWaterPress_vaildMsgCrc16(msg) {
