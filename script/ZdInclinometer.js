//中大无线倾角仪解析
//生成采集指令
function ZdInclinometer_generateInstruct(sensorAddr, sensorType) {
    ...
    var instruct = "00" + sensorAddr + partInstruct + crc16;
    return instruct;
}

// 解析回复指令(默认已去除所有空格)
function ZdInclinometer_parserInstruct(msg) {
    // 判断CRC16是否一致
    if (!ZdInclinometer_vaildMsgLength(msg)){
        return "ERROR01"; //ERROR01 代表长度不够 异常
    }
    if(!ZdInclinometer_vaildMsgCrc16(msg)){
    ...
    var temperature = dataBody.substring(0, 8);
    var dataX = dataBody.substring(8, 16);
    var dataY = dataBody.substring(16);
    var resultTemp = hexToSingle(temperature).toFixed(2);
...
function ZdInclinometer_vaildMsgLength(msg) {
...
function ZdInclinometer_vaildMsgCrc16(msg) {
