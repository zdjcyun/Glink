//上海直川盒式测斜仪解析
//生成采集指令
function ZcBoxSurvey_generateInstruct(sensorAddr, sensorType) {
    var partInstruct = sensorAddr + '0300000006';
    var crc16 = toModbusCRC16(partInstruct, true); //true代表低位在前，false代表高位在前。
    return partInstruct + crc16;
}

// 解析回复指令(默认已去除所有空格)
function ZcBoxSurvey_parserInstruct(msg) {
    // 判断CRC16是否一致
    if (!ZcBoxSurvey_vaildMsgLength(msg)) {
        return "ERROR01" //ERROR01 代表长度不够 异常
    }
    if (!ZcBoxSurvey_vaildMsgCrc16(msg)) {
    ...
    var dataX = dataBody.substring(0, 8);
    var dataY = dataBody.substring(8, 16);
    var resultX = boxMaths(x);
    var resultY = boxMaths(y);
...
function ZcBoxSurvey_vaildMsgLength(msg) {
...
function ZcBoxSurvey_vaildMsgCrc16(msg) {
...
function boxMaths(dataBody) {
    return Math.pow(-1, dataBody.charAt(0)) * ((dataBody.charAt(1) * 100) +
        (parseInt(dataBody.substring(2, 4), 16)) +
        ((parseInt(dataBody.substring(4, 6), 16)) / 100) +
        (parseInt(dataBody.substring(6, 8), 16) / 10000));
}