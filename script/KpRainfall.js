//长沙鲲鹏翻斗式雨量计解析
//生成采集指令
function KpRainfall_generateInstruct(sensorAddr, sensorType) {
    var partInstruct = sensorAddr + '0300010001';
    var crc16 = toModbusCRC16(partInstruct, true); //true代表低位在前，false代表高位在前。
    return partInstruct + crc16;
}

// 解析回复指令(默认已去除所有空格)
function KpRainfall_parserInstruct(msg) {
    // 判断CRC16是否一致
    if (!KpRainfall_vaildMsgLength(msg)){
        return "ERROR01" //ERROR01 代表长度不够 异常
    }
    if(!KpRainfall_vaildMsgCrc16(msg)){
    ...
    var resultX = parseInt(dataBody, 16) / 5;
...
function KpRainfall_vaildMsgLength(msg) {
...
function KpRainfall_vaildMsgCrc16(msg) {
